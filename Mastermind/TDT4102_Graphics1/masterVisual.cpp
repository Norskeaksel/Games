...
{
	const int centerY = 20+round*50; // Regn ut sirklenes y-koordinat (sentrum)
	for (int i = 0; i < size; ++i) {
		const int centerX = mwin.x_max()-80+20*i; // Regn ut sirkelens x-koordinat (sentrum)
		Circle* circle = new Circle{ Point{centerX, centerY}, 8 };
		
		//cout << "i= "<<i << " correctpos= " << correctPosition << " correctCharacter= " << correctCharacter << endl;
		//system("pause");
